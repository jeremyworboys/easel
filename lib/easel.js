...
Easel.prototype.start = function(options) {
    ...
    this.loadModule('database', options);
    this.loadModule('controllers', options);
    this.loadModule('server', options);


    var models = this.models.loadModels();

    this.adapters.bindModels(models);

    this.database.bindModels(models);

    var controllers = this.controllers.loadControllers();
